...
export default function BikesTab() {
    ...
    async function fetchPriceCategories() {
        try {
            const res = await fetch("http://localhost:4000/price-categories");
    ...
    async function handleDeleteBike(id) {
        ...
        try {
            const response = await fetch(`http://localhost:4000/bikes/${id}`, { method: "DELETE" });
    ...
    async function handleSaveBike(e) {
        ...
        if (isEditMode && currentBike.id) {
            url = `http://localhost:4000/bikes/${currentBike.id}`;
            method = "PUT";
        }
